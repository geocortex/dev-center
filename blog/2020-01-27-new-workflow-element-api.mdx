...
-   [Improved form UI consistency](#improved-form-ui-consistency)
...
+ function DemoElement(props: DemoElementProps) {
    ...
    // Previously would be strongly typed. Here we've typed as
    // `number` as declared in our interface above.
...
We've also added `setValue()`. Using the previous API, developers had to remember to raise the `changed` event when updating the `value` property on their element. This was necessary so that workflow authors could run sub-workflows on the `change` event of the element. This is no longer needed as the `setValue()` API will automatically raise the `changed` event for you. `setValue(value)` is equivalent to calling `setProperty("value", value)`.
...
So how would we build this element using the new APIs and ensure that the UI is consistent with the underlying state? Let's take a look:
...
const TextElementRegistration: FormElementRegistration<TextProps> = {
    ...
    onPropertyChange: ({ properties, property, setProperty }) => {
        if (property === "value") {
...
So what does all of this mean in practice? Let's compare an element built using the two APIs:

### Simple Use Case

Here is an example of a simple text input element:
...
class DemoElement extends React.Component<CustomFormElementProps> {
    render(props) {
        return (
            <input
                value={props.element.value}
 ...
 * @description Registers the demo element for use in Display Form
 */
export class RegisterDemoElement extends RegisterCustomFormElementBase {
    execute(): void {
        this.register("Demo", DemoElement);
...
interface DemoElementProps extends FormElementProps<string> {}
...
And here's a more advanced element that displays a range slider with configurable min, max, and step settings:
        ...
        return (
            <div>
                <input
                    max={100}
                    min={0}
                    // Even though we're updating the element value, until the
                    // element is re-rendered the UI won't update.
                    onChange={(event) => {
                ...
                {/* 
                    ...
                    so this UI may not update immediately. `element.value`
                    will be `undefined` initially until set the first time.
                */}
