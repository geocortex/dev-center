...
The [VertiGIS Studio Reporting REST API](https://apps.vertigisstudio.com/reporting/service/api-viewer/index.html?url=../swagger.json#/) can be used to asynchronously run a report, watch for report completion and download the completed report as a PDF.
...
1.  Obtain a run token from the [/auth/token/run](https://apps.vertigisstudio.com/reporting/service/api-viewer/index.html?url=../swagger.json#/auth/post_auth_token_run) endpoint.
    -   Only required if the report is secured or accesses secured content
2.  Run the report with a request to [/job/run](https://apps.vertigisstudio.com/reporting/service/api-viewer/index.html?url=../swagger.json#/job/post_job_run).
...
3.  Use the ticket received in response to the run request to watch for completion of the report via a websocket or polling strategy at [/job/artifacts](https://apps.vertigisstudio.com/reporting/service/api-viewer/index.html?url=../swagger.json#/job/get_job_artifacts).
4.  Use the ticket from the run request and the tag from the artifacts request (step 3) to download the completed report from [/job/result](https://apps.vertigisstudio.com/reporting/service/api-viewer/index.html?url=../swagger.json#/job/get_job_result).
