...
3. Register the activity with Geocortex Workflow and running it using the custom form element.
...
[Register the `RegisterCustomFormElementBase` class in the `IActivityHandlerFactory` for the application](/implement-custom-activity-mobile#register-the-activity-with-the-iactivityhandlerfactory). Now you can [build a workflow for Geocortex Mobile](TODO - what to link to...) that uses your new custom form element!
