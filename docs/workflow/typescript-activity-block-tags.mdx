---
title: TypeScript Activity Block Tags
description: Geocortex Workflow - Learn about the typescript activity metadata tags
---

[Custom workflow activities](implement-custom-activity-web.mdx) implemented in [TypeScript](https://www.typescriptlang.org/) use [block tags](https://jsdoc.app/about-block-inline-tags.html) to describe the activity metadata that [Workflow Designer](https://apps.geocortex.com/workflow/designer/) consumes.

## Basic Tags

The following activity demonstrates the basic metadata tags that could exist on a custom activity and it's inputs and outputs.
...
The following example declares that an activity will only work on Geocortex Web or Web AppBuilder for ArcGIS in an online and client side environment. The display name will default to "Doesnt Work On Mobile".
