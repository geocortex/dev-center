...
description: Geocortex Workflow - Implement a custom activity for web applications that creates a QR code image using a third party library
---
...
export interface CreateQrCodeImageOutputs {
    /** The data URL of the resulting the QR code image. */
...
:::tip
The `result` output of the activity is a data URL. You can use the **Set Form Element Property** activity to assign the URL to the `value` property of an **Image** form element by using the **Set Form Element Property** activity.
:::
