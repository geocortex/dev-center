...
This article applies to building workflow activities and form elements that will be run in [web application workflow hosts](concepts-key-concepts.mdx#geocortex-workflow-hosts).

## Prerequisites

<WebPrereqs />

Follow the instructions in [Implement a Custom Activity](implement-custom-activity-web.mdx) to create a custom workflow activity. You can then import ArcGIS API types with a `require` call.
...
:::note
The `activity-sdk` uses the [ArcGIS API for JavaScript 3.x](https://developers.arcgis.com/javascript/3/) by default. See [Using Alternate ArcGIS API Versions](integrating-the-esri-api-web.mdx#using-alternate-arcgis-api-for-javascript-versions) for details.
:::
...
-   [ArcGIS Web App Builder](https://www.esri.com/en-us/arcgis/products/web-appbuilder/overview) - 3.x ArcGIS API for 2D View, 4.x ArcGIS API for scene view.
...
This example builds on the ExpandPolygon activity to make it work with both the 3.x and 4.x versions of the ArcGIS API. The `execute` method of the activity checks the ArcGIS API version and executes the appropiate code for the given API.

:::note
Since the SDK can only reference one version of the ArcGIS JavaScript API at time, you will have to cast to `any` for the other API version types when neccesary to override TypeScript errors.
:::

In this example, the way the Extent of a polygon is retrieved is the difference between the APIs. In 4.x, `extent` is a property, while in 3.x extent is retrieved using the `getExtent()` method. The API's converge after this, with both supporting the `expand(factor)` method.
