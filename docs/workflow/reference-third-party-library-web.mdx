...
You can reference external JavaScript libraries to augment [custom activities](implement-custom-form-elements-web) or [custom form elements](implement-custom-form-elements-mobile).

## Scope

This article applies to reference third party libraries for TypeScript [activity packs](concepts-key-concepts.mdx#activity-packs) that will be run on [web application workflow hosts](concepts-key-concepts#geocortex-workflow-hosts).

<WebPrereqs />

## Adding a Dependency

1. Install the dependency. For example, `npm install moment.js`.
...
The root that the library path refers to is the `dist` folder, and the `.js` postfix on the library should be omitted.

4. Import and use the library as you normally would. This example demonstrates a custom activity that uses the `moment.js` library to return the current date and time in a human readable format.
...
:::important
If the external dependency has child dependencies, you will need to call `mapDependencies` for each one.
:::
