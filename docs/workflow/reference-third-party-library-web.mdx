...
1. Install the dependency in the activity sdk. For example, `npm install moment.js`.
...
The root that the library path refers to is the `dist` folder, and the `.js` postfix on the library file should be omitted.
