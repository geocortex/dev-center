...
1. Install the dependency. For example, `npm install moment`.
...
The root that the library path refers to is the `dist` folder, and the `.js` suffix on the library should be omitted.
