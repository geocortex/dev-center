...
1. Create a new project of type _Class Library_ (.NET Standard)
    - You can also create a project of type [.NET Core](https://dotnet.microsoft.com/learn/dotnet/what-is-dotnet) to take advantage of it's larger feature set. If so, you need to go into the project settings and change the output type to `Class Library`
