...
Geocortex Workflow Server can be extending by augmenting your on premise installation with additional custom .NET assemblies.
...
1. Create a new project of type _Class Library_ (.NET Standard)
    - You can also create a project of type [.NET Core](https://dotnet.microsoft.com/learn/dotnet/what-is-dotnet) to take advantage of it's larger featureset. If so, you need to go into the project settings and change the output type to `Class Library`
...
Next, learn how to implement a [custom activity](implement-custom-activity-server.mdx) for Geocortex Workflow Server.
...
2. Create a file `excluded_files.txt`at the root of the project that excludes the appropiate build output files as described in the [deployment section](#deploying-geocortex-workflow-server-activities).
...
3. Run a rebuild and ensure all appropiate files are copied.
...
Check out the use cases to learn how to build custom activities for Geocortex Workflow Server and augment them with third party libraries.
