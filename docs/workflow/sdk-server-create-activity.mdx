...
Follow the instructions to [deploy VertiGIS Studio Workflow Server activities](sdk-server-overview.mdx#deploying-geocortex-workflow-server-activities).

## Use the Activity in a Workflow

[Server workflows](https://docs.geocortex.com/workflow/latest/help/Default.htm#wf5/help/server-workflows.htm) that run on your VertiGIS Studio Workflow Server deployment can now run this custom activity.
...
You can run the custom activity by referencing it by action with [the `RunActivity` activity](https://docs.geocortex.com/workflow/latest/help/Default.htm#wf5/help/activities/run-activity.htm). The custom activity created in this article takes no inputs but produces an output with the property "test".
