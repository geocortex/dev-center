...
Implementing a custom [form element](concepts-key-concepts.mdx#activities-and-form-elements) allows you to build a custom interactive form component to augment the existing [form elements that come with Geocortex Workflow](https://docs.geocortex.com/workflow/latest/help/Default.htm#wf5/help/form-elements/auto-complete.htm%3FTocPath%3DForms%7CForm%2520Element%2520Reference%7C_____0).

## Scope

This article applies to building workflow form elements that will be run on [web application workflow hosts](concepts-key-concepts#geocortex-workflow-hosts).

<WebPrereqs />

It is suggested that you [implement a custom activity](implement-custom-activity-web.mdx) before trying to implement a custom form element.

:::important
Custom workflow form elements are built in [React](https://reactjs.org/).
:::
...
3. Register the activity with Geocortex Workflow and running it **before** using the custom form element.
...
[Build and deploy](getting-started-web.mdx#building) your new [activity-pack](concepts-key-concepts.mdx#activity-packs). Now you can [build a workflow for Geocortex Web](TODO - what to link to...) that uses your new custom form element!
...
```typescript
export class CustomFormElement extends React.Component<CustomFormElementProps> {
    ...
    handleClick = event => {
        const { element, raiseEvent } = this.props;
...
You can then use an expression in Geocortex Workflow Designer to access the custom property of the form element. For example, to access `value` and `data` properties use an expression like `=$form1.state.custom1.value` and `=$form1.state.custom1.data`.
