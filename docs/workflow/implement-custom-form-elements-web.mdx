...
const SfcCustomFormElement = (props: CustomFormElementProps) => {
...
You can then use an expression in Geocortex Workflow Designer to access the custom property of the form element. For example, to access the `value` property you can use an expression like `=$form1.state.custom1.value`.
