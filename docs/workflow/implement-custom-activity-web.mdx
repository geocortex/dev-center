...
The `npm run activity` script creates a custom activity with some dummy inputs and outputs. It also has examples of the various [block tags](typescript-activity-block-tags.mdx) that are used to describe activity metadata. By changing the inputs, outputs, execute logic, and tags, you can create your own custom activity that has an easy to use interface in the Geocortex Workflow Designer.
