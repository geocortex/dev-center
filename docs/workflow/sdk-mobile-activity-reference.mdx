...
description: Geocortex Workflow - Reference for workflow activities for Geocortex Mobile
---
...
You can access Geocortex Mobile functionality in custom workflow activities by using the built-in [dependency injection pattern](../mobile/sdk-dependency-injection.mdx) to inject shared Geocortex Mobile [services](../mobile/concepts-components-services.mdx#services) such as the `UIOperations` service.
...
Next, [inject the activity](../mobile/sdk-dependency-injection.mdx) into the [`IActivityHandlerFactory` for the application](tutorial-mobile-calculate-logarithm-activity.mdx#register-the-activity-with-the-iactivityhandlerfactory).
...
The map of the host Geocortex Workflow application can be accessed through [dependency injection ](../mobile/sdk-dependency-injection.mdx). The `MapProviderBase` class which exposes the [Map](https://developers.arcgis.com/net/latest/uwp/api-reference/html/T_Esri_ArcGISRuntime_Mapping_Map.htm) and [MapView](https://developers.arcgis.com/net/latest/uwp/api-reference/html/T_Esri_ArcGISRuntime_UI_Controls_MapView.htm) can be injected with the `ProviderFactoryAttribute`.
