...
Learn how to build a [custom activity](implement-custom-activity-mobile.mdx) for Geocortex Mobile before reading this article.

## Access Geocortex Mobile Functionality

Accessing Geocortex Mobile functionality in custom workflow activities uses the built in [dependency injection pattern](../mobile/dependency-injection.mdx) to inject shared Geocortex Mobile [services](../mobile/concepts-components-services.mdx) such as the `UIOperations` service.
...
2. [Inject the class into the `IActivityHandlerFactory` for the application](/implement-custom-activity-mobile#register-the-activity-with-the-iactivityhandlerfactory), and pass it to your custom activity's constructor.
...
namespace App1.Workflow
{
    public class DisplayGeocortexMobileAlert : IActivityHandler
    {
        ...
        public const string Action = "custom:wf:app::DisplayGeocortexMobileAlert";
...
Accessing the map for the application uses the built in [dependency injection pattern](../mobile/dependency-injection.mdx) to inject a `MapProviderBase` class which exposes the [Map](https://developers.arcgis.com/net/latest/uwp/api-reference/html/T_Esri_ArcGISRuntime_Mapping_Map.htm) and [MapView](https://developers.arcgis.com/net/latest/uwp/api-reference/html/T_Esri_ArcGISRuntime_UI_Controls_MapView.htm).

1. Add a new [autofac factory](https://autofaccn.readthedocs.io/en/latest/resolve/relationships.html#dynamic-instantiation-func-b) property that produces a `MapProviderBase` to your custom activity.
...
namespace App1.Workflow
{
    ...
    {
        public const string Action = "custom:wf:app::PlaceCalloutAtCenter";
        ...
        public async Task<IDictionary<string, object>> Execute(IDictionary<string, object> inputs, IActivityContext context)
        {
            ...
            MainThread.BeginInvokeOnMainThread(() =>
            {
                ...
                mapView.ShowCalloutAt(currentEnvelope.GetCenter(), new CalloutDefinition("Hey!", "Listen!"));
            });
