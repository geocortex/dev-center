...
Accessing Geocortex Mobile functionality in custom workflow activities uses the built-in [dependency injection pattern](../mobile/sdk-dependency-injection.mdx) to inject shared Geocortex Mobile [services](../mobile/sdk-services-overview.mdx) such as the `UIOperations` service.
