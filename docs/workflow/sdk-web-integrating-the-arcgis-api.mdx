...
For applications like [Geocortex Web](https://www.geocortex.com/products/gxw/) that use the 4.x ArcGIS API for JavaScript use:

```sh
npm install @types/arcgis-js-api@4.17.0
```

For applications like [Geocortex Viewer for HTML5](https://www.geocortex.com/products/geocortex-essentials/) or [Web AppBuilder for ArcGIS](https://www.esri.com/en-us/arcgis/products/web-appbuilder/overview) (2D) that use the 3.x ArcGIS API for JavaScript use:
...
export class ExpandPolygon {
    static action = "uuid:<uuid>::ExpandPolygon";
...
Geocortex Workflow is designed to work with both the 3.x and 4.x versions of the ArcGIS API for JavaScript; however, many custom activity implementations only bother to support one or the other.
