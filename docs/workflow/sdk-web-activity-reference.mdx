...
In the Geocortex Workflow JavaScript SDK, activities are represented as simple classes with an `execute` method.
...
The Geocortex Workflow JavaScript SDK has a concept of an `AppActivity`, which is an activity that needs access to it's [host](concepts-key-concepts.mdx#geocortex-workflow-hosts).
