...
The VertiGIS Studio Workflow TypeScript SDK has a concept of an `AppActivity`, which is an activity that needs access to its [host](key-concepts.mdx#geocortex-workflow-hosts).
