...
:::tip
Geocortex Web has many built-in [Commands and Operations](concepts-commands-operations.mdx) which can be [chained together](sdk-commands-operations.mdx#command-and-operations-in-app-config) to build custom behaviors with. If these commands and operations are not enough to accomplish your goal, you can use [Geocortex Workflow](usecases-run-workflow-app-config.mdx) to take custom behavior even further without writing custom code. If Workflow is still not enough to accomplish your needs, then it might be time to implement a custom command or operation.
:::
...
You can test your custom command and operation by adding entries to the I Want to Menu in the [app config](concepts-app-config.mdx) that run your commands and operation. For more information on customizing your app with commands and operations, see the [commands and operations reference](sdk-commands-operations.mdx).
