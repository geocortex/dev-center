...
It's now time to implement the command to record an action, and the operation that returns a history of executed actions in the `CustomService`. We can define each command and operation with an appropriately named public method.
