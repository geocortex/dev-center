...
Following the best practices for implementing components, the news item data should be managed by the news feed model. Component Models can [inject services](sdk-components-referencing-services.mdx) as properties, and doing so allows us to directly access the dynamically updated `newsItems` property on the service.
