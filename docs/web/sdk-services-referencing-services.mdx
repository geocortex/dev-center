...
The property that the `@inject` decorator is bound to will be populated during the [initialization process](sdk-component-reference.mdx#initialization-and-teardown) of the service that the decorator was used in. The property value will be `undefined` until the service is initialized.
