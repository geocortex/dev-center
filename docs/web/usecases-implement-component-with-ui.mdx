...
Implementing a custom component in Geocortex Web gives you one of the highest degrees of flexibility with what you can do in Geocortex Web. [Components](concepts-components-services.mdx) can display custom, dynamic UI, register command and operation implementations, store persistent data, and more. They are also one of the most complex ways of customizing your Geocortex Web app, so it's worthwhile to try simplier options like [changing the default behaviour](usecases-change-default-behavior.mdx) or [Geocortex Workflow](usecases-change-default-behavior.mdx#using-workflows-to-customize-behaviour) first.
...
The first thing we need to do is create the neccesary files for a custom component. Every component needs an associated model that extends `ComponentModelBase`.
