...
If you run the [development server](sdk-overview.mdx#getting-up-and-running) you should see your custom component up and running.
...
The following example demonstrates a component that uses Geocortex UI Framework components and an icon from the [Geocortex Icons library](TODO-icon-library-link).
...
When writing custom components or [custom services](usecases-implement-service.mdx), you may want to take advantage of Geocortex Web's large [built in suite of command and operations](api-commands-operations.mdx) or your own [custom commands and operations](usecases-implement-command-operation.mdx). You can run command and operations through the `UIContext` of the component.
