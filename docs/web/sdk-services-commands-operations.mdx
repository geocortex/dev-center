...
Services can also implement commands and operations. By decorating a method with `@command` or `@operation`, services can define an implementation for a command or operation. The command or operation must also be registered through the `registerOperation` or `registerCommand` methods in the library registry with the appropriate service ID.
