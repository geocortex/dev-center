...
Services can also register an implementation for a command or operation. A command or operation is registered through the `registerOperation` or `registerCommand` registry methods. By decorating a method with `@command` or `@operation`, services can define an implementation for the registered command or operation.
