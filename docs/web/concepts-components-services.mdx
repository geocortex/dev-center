...
Every component has an associated model. This model functions as the underlying data source for the component, as well as the middleware for interfacing with the larger application, through [commands, operations](concepts-commands-operations.mdx) and shared services. This allows the component to solely worry about presentation concerns, and delegate data and application concerns to the model.

## Services

In Geocortex Web, services represent a shared global concern that needs to be accessed by many components or other services. There is only one instance of each service in the application. Interactions with services usually happen through the [commands and operations they implement](implement-command-operation.mdx#implement-the-custom-command-and-operation), but services can also be directly [injected](TODO-injection) into services and component models.
