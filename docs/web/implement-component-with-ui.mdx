...
The following component uses it's model to call the `ui.confirm` command and `ui.display-notification` operation. Calling a command or operation is as simple as retrieving it by name using the `messages` property on the model, and then calling `execute` with the appropiate arguments.
