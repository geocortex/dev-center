...
The branding [service](concepts-components-services.mdx#services) is responsible for the theming of the application. To select a theme and accent color from within the config, you can configure the branding service in the [app config](concepts-app-config.mdx). The `template` property is used to specify the base theme to seed colors from.
...
How elements are laid out in a Geocortex Web Application is defined by an app's layout. [This article](concepts-layout.mdx) will teach you how to configure an app's layout and how to leverage it to customize the appearance of Geocortex Web. Further styling within the layout can be done using [presentation attributes](api-layout.mdx#presentation-attributes).
