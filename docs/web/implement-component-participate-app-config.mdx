...
Geocortex Web has a powerful [app configuration](concepts-app-config.mdx) model which can be used to easily change the behaviour of an application without modifying custom code. Using app config to power a components behaviour increases it's reusability and customizability.
