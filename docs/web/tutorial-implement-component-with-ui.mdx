...
Check out a [live SDK sample](https://vertigis-web-samples.netlify.app/basic-component) that demonstrates implementing a custom component. You can also view our [UI library sample](https://vertigis-web-samples.netlify.app/ui-library) that shows how to use some of the available Geocortex Web React UI components. These components allow you to take advantage of styled, well tested, UI components that already participate in the [application theme](configuration-theme.mdx).
