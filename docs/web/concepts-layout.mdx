...
The examples above first declares a `<map>` component that uses the default map configuration.
Next, a `<zoom>` component is added **within** the map component. This does two things:

1. Visually places the element inside the map
2. Tells the zoom widget to act upon [the map it is placed in](api-component-models.mdx)
...
For a deeper dive into stacks and splits, [check out the API reference](api-layout-core-components.mdx#stacks-and-splits).

## Spacing

See [presentation attributes](api-layout-attribute-reference.mdx#presentation-attributes) for details on margin, padding, content organization and alignment in your layout.
...
Components can react to changes or mutate values on other components through these models. For example, if the scale of the map changes when it is zoomed, then the scalebar will react to the change in the map model and update it's displayed values.

For more information on models, [check out the component SDK reference](sdk-component-reference.mdx#models) and this deeper dive into [model binding in the layout](api-component-models.mdx).
...
When adding custom components, you will need to [add their custom namespace](sdk-component-reference.mdx#components-and-layout) to your xml definition.
