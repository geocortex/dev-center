...
However, in the following example with two scale bars, how does each scale bar known which map model it corresponds to? Both are placed outside of a `<map>` component, and there are two possible map models to bind to. 
