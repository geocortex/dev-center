...
:::note
If you do not provide a `config` attribute for a component, Geocortex Web will attempt to [infer a default value](sdk-component-reference.mdx#component-defaults).
:::
...
This example demonstrates a web application with a hidden side panel that pops out when you click a button. It was created entirely with app config and layout. Some [advanced layout concepts](concepts-advanced-model-binding.mdx) and [advanced app config concepts](concepts-advanced-app-config.mdx), such as linking config items through [item URIs](concepts-advanced-app-config.mdx#item-uris), were used.
...
<UseCaseContainer>
    ...
    <UseCaseCard
        ...
        link="concepts-advanced-app-config"
    />
    <UseCaseCard
        title="Explore Geocortex Web's built-in components"
        description="Dive into Geocortex Web's rich component library in the SDK"
        link="api-components"
    />
</UseCaseContainer>
