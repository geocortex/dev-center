...
This example demonstrates a web application with a hidden side panel that pops out when you click a button. It was created entirely with app config and layout. Some concepts from [advanced layout](concepts-advanced-layout.mdx) and [advanced app config](concepts-advanced-app-config.mdx), such as linking config items through [item URIs](concepts-advanced-app-config.mdx#item-uris), were used.
