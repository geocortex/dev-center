...
:::warning
The Geocortex Web SDK is currently in preview. The patterns for development and deployment are still under active development and are subject to change. We don't yet recommend deploying your custom code into production until we've finalized the SDK.
:::

If the built in configurability of Geocortex Web does not serve your purposes, then it might be time to dive into the Geocortex Web SDK. Building applications on top of the Geocortex Web SDK requires enaging in the development, testing, and deployment of custom code, so it's worthwhile to make sure your problem cannot be solved [with workflow or other advanced configuration](overview.mdx#explore-the-use-cases).
