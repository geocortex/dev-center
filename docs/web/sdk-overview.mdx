...
The Geocortex SDK provides a development toolkit for building [custom components](sdk-create-component.mdx), [services](sdk-create-service.mdx), and more into a package that can be deployed to Geocortex Web called a _library_. Once the _library_ has been developed, it can then be [deployed](sdk-deployment.mdx) to your SaaS or on-premises environment.
...
Commands and operations provide a framework for executing application wide behavior. Services and components can both run and implement commands and operations, allowing for interaction and consistent behavior without tight coupling. Commands and operations can also be run [in the app config](usecases-run-command-with-button.mdx) and by [Workflows](https://docs.geocortex.com/workflow/latest/help/Default.htm#wf5/help/activities/run-command.htm). This can allow you to [customize the behavior of built-in components](usecases-change-default-behavior.mdx).
...
-   Component and service interaction through commands and operations is preferred, but components and services can also be tightly coupled to each other through [model injection](sdk-component-interactions.mdx#importing-models) or [service injection](sdk-service-injection.mdx).
