...
Next, add a [app item](configuration-app-config-getting-started.mdx#anatomy-of-an-app-config) to the configuration for the map component, and configure the map to run `ui.display-notification` on initialization as a test.
...
<UseCaseContainer>
    ...
    <UseCaseCard
        ...
        link={useBaseUrl("docs/web/tutorial-implement-custom-form-elements")}
    />
    <UseCaseCard
