...
Geocortex Web has a large suite of [built in events](api-events.mdx). Any of these events can be subscribed to by any component or service. The events are accessed through the [`MessageBus`](TODO-api-concepts)
    ...
    protected _onInitialize(): Promise<void> {
        this.handles.push(
            this.messages.events.map.initialized.subscribe((e: MapEvent) => {
                console.log("Map Initialized, do your map dependent model setup here.");
            })
