...
export default function CustomComponent(props) {
    ...
    const displayConfirmDialog = async () => {
        const result = await operation<ConfirmOperationArgs, boolean>("ui.confirm")
                            .execute({
                                title: "Confirm Me",
                            });
...
Components can also register an implementation for any command. This allows for multiple actions to occur on a command execution, and a tighter integration with side effects in the UI. 

### Example: Component that Implements a Command

When registering a command implementation for a component, it's best practice to register the implementation in the components model. The following component model registers an implementation for a custom command that takes a `string` argument.

:::tip
If necessary, a component could also register the command implementation through the [UIContext](sdk-ui-context).
:::
    ...
    protected async _onInitialize(): Promise<void> {
        ...
        this._handles.push(
            this.messages
                ...
                .register((arg: string) => {
                    console.log(`Executing 'custom.my-command' with argument '${arg}'`)
                    return;
                })            
        );
    }
