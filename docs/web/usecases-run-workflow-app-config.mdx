...
Next, add a [app item](concepts-app-config.mdx#anatomy-of-an-app-config) to the configuration for the map component, and configure the map to run `ui.display-notification` on initialization as a test.
...
<img
    src={useBaseUrl("img/web-workflow-url-map-initialization.png")}
