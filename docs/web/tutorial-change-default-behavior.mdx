...
:::tip
Many behaviors which are configurable through the app config are more easily configurable by using the [VertiGIS Studio Web Designer](https://apps.geocortex.com/webviewer/designer/). App config should be used if you need to modify a property not available in the app config, or use a custom command as part of a [command chain](configuration-commands-operations.mdx#configuring-commands-and-operations).
:::
