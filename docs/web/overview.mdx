...
If the Geocortex Web Designer cannot accomplish what you need, you can configure and extend Geocortex Web in a number of ways.

<ExtensibilitySpectrum />

### Application Configuration

The simplest way to customize a Geocortex Web Application beyond what is available through the Geocortex Web Designer is through the app config and layout. Every Geocortex Web application has a [**layout**](configuration-layout.mdx), which represents the visual layout of the application, and [**app config**](configuration-app-config.mdx) that configures various behaviors and settings within the application. Layout and app config can be easily edited to customize the appearance and behavior of an an Geocortex Web application without writing custom code.

[Explore the concepts](key-concepts.mdx) to learn more about layout and app config.

### Geocortex Workflow

<!-- TODO - does Geocortex Workflow come bundled with Geocortex Web? -->

[**Geocortex Workflow**](https://docs.geocortex.com/workflow/) is a powerful tool that comes bundled with Geocortex Web which can create custom behavior and UI without custom development. Many behaviors in Geocortex Web can be [replaced by a workflow](tutorial-change-default-map-click-behavior.mdx) that runs entirely custom logic. Geocortex Workflow can even [display complex customized forms](https://docs.geocortex.com/workflow/latest/help/Default.htm#wf5/help/forms.htm).

### Custom Workflow Activities and Form Elements

If Geocortex Workflow meets most of your need, but is missing a few crucial elements, the best solution may be to extend Geocortex Workflow with **[custom activities](usecases-web-implement-custom-activity.mdx)** and **[form elements](usecases-web-implement-custom-form-elements)**. Extending Geocortex Workflow for Geocortex Web requires some custom development, but is simpler than developing entirely custom logic and UI with the Geocortex Web SDK.

### Custom Code

If you need to build entirely custom and new behavior and UI, you should check out the [**Geocortex Web SDK**](sdk-overview.mdx). Using the Geocortex Web SDK, you can develop custom behavior by creating new [commands and operations](tutorial-implement-command-operation.mdx) which can be [used in the app config](configuration-commands-operations.mdx). If you need to create a new UI element, you can build [custom components](sdk-create-component.mdx). With the ability to create [background services](tutorial-service-dynamic-data.mdx), facilitate [component interactions](sdk-component-interactions.mdx), and [participate in the app config](tutorial-implement-component-participate-app-config.mdx), the possibilities for extending Geocortex Web are endless.
