...
:::note
If a [custom command or operation](TODO-link-custom-command) can accomplish the behavior you need without a direct service reference, this is preferred to decrease the degree of coupling.
:::
...
Note that the `@inject` decorator will automatically inject a reference to the service only when used from within a component model or service. The property that the `@inject` decorator is bound to **will be populated during the [initialization process](TODO-initialization-link) of the given component model or service** that the decorator was used in. The property value will be `undefined` until the component model or service is initialized.
