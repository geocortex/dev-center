...
The second command/operation in this application is a `map.zoom-to-initial-viewpoint` command on the I Want To Menu. This command takes a `MapExtensionArgs`, which is provided as a named argument. The `maps` property in the argument references the `default`map in the config through an [**Item Uri**](concepts-advanced-config.mdx#item-uris). Item URI's are a way of referencing other items within the app config.
