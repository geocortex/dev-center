...
Commands and operations are used to power much of Geocortex Web's built in behavior and interactions. They can be run through [configuration](usecases-run-command-with-button.mdx), or through [custom services](sdk-services-commands-operations.mdx) or [components](sdk-components-commands-operations.mdx). They power everything from basic components like the IWTM, to advanced functionality like the results list.
