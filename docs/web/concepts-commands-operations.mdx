...
This will override the default argument passing behavior for that command or operation. For example, `ui.display-notification` takes an object of type `DisplayNotificationArgs`, which has the property `message` of type `string`.
