...
[Custom components](sdk-create-component.mdx) often need to access the application context to run [commands and operations](concepts-commands-operations.mdx), subscribe and publish to [events](concepts-events.mdx), and more. A component has two ways of accessing the application context.

First, every component has a [model](sdk-component-reference.mdx#models) which in turn has a [`MessageBus`](api-message-bus.mdx) that can be used to access commands, operations, and events. All application context interactions can be facilitated by the model.
