...
[Custom components](usecases-implement-component-with-ui.mdx) often need to access the application context to run [commands and operations](concepts-commands-operations.mdx), subscribe and publish to [events](concepts-events), and more. A component has two ways of accessing the application context.
...
export default function CustomComponent(props) {
    const { command, event, operation, translate } = useContext(UIContext);
    // ...
}
```
