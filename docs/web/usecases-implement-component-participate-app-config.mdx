...
We are going to add configuration to a custom component that displays news items. This component currently is following bad practices and does not treat the model as the source of truth for it's data. We are going to move the `newsItems` list to a configurable property on the model, and add a new configurable property, `hideOnStartup`.
