...
Finally, we need to update the `NewsFeed` to treat the model as it's single source of truth for data. First, we update the props passes into the component to include the relevant model.
