...
Finally, we need to update the `NewsFeedComponent` to treat the model as its single source of truth for data. First, we update the props passes into the component to include the relevant model.
