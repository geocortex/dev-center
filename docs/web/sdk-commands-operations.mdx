...
description: Geocortex Web - Reference for commands and operations in the SDK
---
...
-   Commands such as `map.zoom-to-viewpoint` execute behavior that can have effects on the application.
-   Operations such as `results.convert-to-csv` produce outputs which can be passed to commands.
-   Commands and operations can be run sequentially in what's called a "command chain" to enable complex behavior.
-   Built in components have properties in the app config which take a command or operation as values, allowing their internal behavior to be configured.
...
[Services](sdk-services-overview.mdx) or [components](sdk-components-overview.mdx) are usually responsible for registering a command or operation implementation with Geocortex Web. Once a command or operation has been registered, it can be used from anywhere in the application.
...
<UseCaseContainer>
    <UseCaseCard
        title="Configure a Button to Run a Command or Operation"
        description="Follow a step by step guide to configure a button with a command or operation"
        link="usecases-configure-button-click.mdx"
    />
    <UseCaseCard
        title="Change Default Behavior with Commands and Operations"
        description="Follow a step by step guide to change built-in component behavior with commands and operations"
        link="usecases-change-default-behavior"
    />
    <UseCaseCard
        title="Learn more about Commands and Operations in Services"
        description="Learn more about how commands and operations interact with services"
        link="sdk-services-commands-operations"
    />
    <UseCaseCard
        title="Learn more about Commands and Operations in Components"
        description="Learn more about how commands and operations interact with components"
