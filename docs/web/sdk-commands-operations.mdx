...
Components and services can also implement commands and operations. This allows the creation of commands which can affect the UI. Command and operation implementations are defined in a [component's model](sdk-components-reference.mdx#models) or in a [service](sdk-services-overview.mdx). By decorating a method with `@command` or `@operation`, components and services can define an implementation for a command or operation. The command or operation must also be registered through the `registerOperation` or `registerCommand` methods in the library registry with the same item type that the model was registered with.

The following component model defines an implementation for a custom command that takes a custom argument type, and a custom operation that returns a custom argument type. The same patterns apply when defined within a service.
