...
This is a reference for the attributes available to all components in an [app layout](configuration-layout.mdx). Some of these attributes are accessible through the [Geocortex Web Designer](https://apps.geocortex.com/webviewer/designer/), and the rest must be configured through layout.
...
**Type**: `string`  
**Description**: A css selector string specifying the component with the model that needs to be resolved. See [models](configuration-layout.mdx#models) for details.
