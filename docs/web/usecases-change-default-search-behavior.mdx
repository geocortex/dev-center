    ...
    "items": [
        {
            ...
            "tableExtensions": [
              "item://table-extension/fire-hydrants-table"
            ],
            "onClick": [
...
The first thing we need to do is override the default search behavior. We can do this by [editing the default behaviour](usecases-change-default-behavior) for the "Search" component in the app config. The default behavior for search is a command chain that runs `tasks.search` and then `results.display`, as seen in the [api](api-components.mdx#search).
