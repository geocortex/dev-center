...
Components can interact with each other through their [respective models](sdk-component-reference.mdx#models). A model can either declare that it is exported, like the map model, or that it imports a model.

-   Declaring a model as exported exposes it to other components for importing.

-   Using the `@importModel` decorator will tell the component to attempt to retrieve a reference to the given model type, according to the [model binding rules](concepts-advanced-model-binding.mdx)
...
<UseCaseContainer>
    <UseCaseCard
        ...
        link="usecases-import-map-model"
    />
    <UseCaseCard
