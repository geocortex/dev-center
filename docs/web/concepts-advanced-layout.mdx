...
:::note
This import logic also applies to the [models](concepts-layout.mdx#models) attribute. If a corresponding `models` property is found before a component with a model during the layout search, the model referenced by the `models` attribute will be imported instead. For the above example, adding a `models="#map1"` attribute to the panel containing the `<search>` component would be a flexible solution to import the correct map into all the sub-components.
