...
All services need to be registered with Geocortex Web. Services can optionally use the `itemType` property to bind to an app item in config.
...
All services have access to initialization and cleanup methods, as well as the global message bus that allows for the registration and execution of commands and operations.
...
export default class ExampleService extends ServiceBase {
    protected async someMethod(): Promise<void> {
        ...
        command<string>("my-custom.command").execute("some arg");
        ...
        // You must provide the correct generic typings.
...
On application load, all services that are configured to automatically load on startup are created and initialized **in parallel**, along with core framework services like the message bus.
