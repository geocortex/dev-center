...
When the subscriber calls the `this.messages.events("my.custom-event")` function, this creates the event if it has not already been created. It can then be immediately subscribed to. When the publisher calls `this.messages.events("my-custom-event")`, it will reference the same event that the subscriber created.
