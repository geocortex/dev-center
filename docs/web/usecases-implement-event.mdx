...
Events can only be published and subscribed to by [custom components](usecases-implement-component-with-ui.mdx) and [custom services](usecases-service-dynamic-data.mdx), so it is suggested you learn the basics of building custom components before reading this article.
