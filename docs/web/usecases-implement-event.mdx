...
Geocortex Web has a large suite of [built-in events](api-events.mdx). Any of these events can be subscribed to by any component or service. The events are accessed through the [`MessageBus`](api-message-bus.mdx) in the case of a service or model, or the [UIContext](sdk-ui-context.mdx) in the case of a component.
...
export default function CustomComponent(props) {
    ...
    var [handles, setHandles] = useState<IHandle[]>([]);

    useEffect(() => {
        setHandles(
            handles.concat(
                events.map.initialized.subscribe((e: MapEvent) => {
                    console.log(
