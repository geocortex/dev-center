...
Many of the behaviours of Goecortex Mobile's built in [components](concepts-components-services.mdx) have configurable behaviours through the [app config](concepts-app-config.mdx). If you browse the `app.json` that is included with the [Quick Start](quick-start.mdx), you'll see properties like `action` or `onClick` in items that take an array of [commands and operations](concepts-commands-operations.mdx).
...
All behaviours defined in this manner are configurable. Commands or operations can be removed from the chain, and any behaviour can be replaced with a `workflow.run` command that [executes custom behaviour](change-default-search-behaviour.mdx), or even [custom commands and operations](implement-command-operation.mdx).
