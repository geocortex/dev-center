...
All behaviours defined in this manner are configurable. Commands or operations can be removed from the chain, and any behaviour can be replaced with a `workflow.run` command that [executes custom behaviour](change-default-search-behaviour), or even with [custom commands and operations](implement-command-operation).
