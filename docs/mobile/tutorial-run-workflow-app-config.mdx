...
:::important
Configuring a workflow to run when the map is initialized can be done through [Geocortex Mobile Designer](https://apps.geocortex.com/mobileviewer/designer/) and this is the preferred method of configuration if Designer can be used. The goal of this article is to demonstrate the pattern of overriding behaviors with workflows.
:::
