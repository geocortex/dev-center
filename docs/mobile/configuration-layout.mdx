...
2. Tells the zoom widget to act upon [the map it is placed in](configuration-advanced-layout.mdx#nested-model-imports)
...
:::tip
The [Quickstart](sdk-getting-started.mdx) comes with a schema for the `layout-*.xml` files which can [autocomplete slot values for a component](configuration-advanced-layout.mdx#autocomplete-properties)
:::
...
:::important
The [Quickstart](sdk-getting-started.mdx) does not ship with a medium layout configured. See [Add Medium Layout](tutorial-add-medium-layout.mdx) for instructions on how to do this.
:::
...
When adding custom components, you will need to [add their custom namespace](tutorial-implement-component-with-ui.mdx#add-the-component-to-your-layout) to your xml definition.
...
<UseCaseContainer>
    ...
    <UseCaseCard
        ...
        link="configuration-advanced-layout"
    />
</UseCaseContainer>
