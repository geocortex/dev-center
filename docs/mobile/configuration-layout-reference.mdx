...
This is a reference for the attributes available to all components in an [app layout](configuration-layout.mdx). All components support these attributes, as well as any custom attributes the component defines. Some of these attributes are accessible through the [Geocortex Mobile Designer](https://apps.geocortex.com/mobileviewer/designer/), and the rest be configured through layout.
...
**Type**: `string`  
**Description**: Used to bind a component to an item in the app config. See [linking layout app config](configuration-app-config.mdx#linking-layout-to-app-config) for more details.
