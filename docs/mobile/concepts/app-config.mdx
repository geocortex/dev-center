...
App "items" can be components, such as menus or maps, portal items, and more. All item app configuration is specified within the `items` array of the `app.json` file.
...
2. A menu `$type` item which provides config for the I Want To Menu in the layout.
    * In the layout, the `<itwm>` component has the config attribute set with the value `iwtm`. Connecting components in this way allows you to loosely couple UI to content.
