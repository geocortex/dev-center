...
This is a reference for the attributes available to all components in an [app layout](concepts-layout.mdx). All components support these attributes, as well as any custom attributes the component defines. Some of these attributes are accessible through the [Geocortex Mobile Designer](https://apps.geocortex.com/mobileviewer/designer/), and the rest must be configured through layout.
