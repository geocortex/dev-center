...
Althought it is best practice to define an operations class that extends `OperationsBase`, you can register a command or operation from anywhere you can [inject](sdk-dependency-injection.mdx) the `IOperationRegistry`. For example, this service registers an new operation `custom.alert` without defining an `OperationsBase` class.
