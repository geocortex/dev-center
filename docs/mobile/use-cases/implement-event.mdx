---
id: implement-event
title: Implement a Custom event
---
...
:::info
Geocortex Mobile has many built in [Events](/docs/mobile/concepts/events) that can be used to hook into various changes that occur in the application. Check out the [Events API](/docs/mobile/api/events) to see if the event you need already exists.
:::

This article will walk you through implementing a custom Event in Mobile that runs every five seconds.
...
TODO note on naming conventions?

:::info
The two key components of this snippet are the `[assembly: Export(typeof(CustomEvents), SingleInstance = true, ...` tag and the class extension `AlertService: EventsBase`. These two pieces register the Service with Mobile and allow it to be injected with Autofac
:::
...
Since the `CustomEvents` class was registered with Autofac as a singleton, any other Component, Service, or View that injects it can publish or subscribe to the event. The example below but demonstrates how events could be used to tell views and services that rely on shared data that it has changed.

:::important
If you haven't yet, check out [Implementing Custom Components](/docs/mobile/use-cases/implement-custom-component-with-ui) and [Implementing a Custom Background Service](/docs/mobile/use-cases/implement-background-service) before diving into this example 
:::
...
<Tabs
  ...
  values={[
    ...
    { label: 'View - Code Behind (Consumer)', value: 'code-behind', },
    { label: 'Service (Consumer)', value: 'service', },
...
<TabItem value="ui">
    <DemoVideo src="/video/event-timer.webm"/>
</TabItem>
</Tabs>

One thing to note about this example is the use of `IDisposable` and `IDisposableTracker` to clean up event handlers in the View code behind and Service. Implementing these interfaces correctly is essentially to preventing [Memory Leaks in your Mobile Application](TODO - memory leaks link).