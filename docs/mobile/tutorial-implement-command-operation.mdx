...
:::tip
Geocortex Mobile has many built-in [Commands and Operations](configuration-commands-operations.mdx) which can be chained together to build custom behaviors. If these Commands and Operations are not enough to accomplish your goal, you can use [Geocortex Workflow](tutorial-change-default-map-click-behavior.mdx) to take custom behavior even further without writing custom code. If Workflow is still not enough to accomplish your needs, then it might be time to implement a Custom Command / Operation.
:::
...
It's convention that commands and operations in Geocortex Mobile are registered and implemented by a [service or component](configuration-components-services.mdx), so the first thing we will do is create a service.

1. Create a new file `Services/AlertService.cs` under the platform agnostic project.
    - In the file, add a new service class `AlertService` and [register it with **Autofac**](sdk-dependency-injection).
