...
Check out the [Quickstart](quick-start.mdx) and [edit the default app layout](quick-start.mdx#editing-the-layout-and-app-config).
:::
...
:::tip
The [Quickstart](quick-start.mdx) comes with a schema for `layout.xml` which can [autocomplete slot values for a component](concepts-advanced-layout.mdx#autocomplete-properties)
:::
...
:::tip
The default `layout-large.xml` in the Geocortex Mobile [quickstart](quick-start.mdx) has examples of multiple panels that live in the taskbar.
:::
...
:::tip
The Geocortex Mobile SDK [quickstart](quick-start.mdx) includes a large and small layout.
:::
...
:::important
The [quickstart](quick-start.mdx) does not ship with a medium layout configured. See [Add Medium Layout](add-medium-layout.mdx) for instructions on how to do this.
:::
