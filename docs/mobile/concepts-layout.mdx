...
:::tip
The [Quick Start](quick-start) comes with a schema for `layout.xml` which can [autocomplete slot values for a component](concepts-advanced-layout#autocomplete-properties)
:::
...
:::important
The [quick start](Quick Start) does not ship with a medium layout configured. See [Add Medium Layout](add-medium-layout) for instructions on how to do this.
:::
