...
:::tip
The [Quick Start](sdk-quick-start.mdx) comes with a schema for `layout.xml` which can [autocomplete slot values for a component](concepts-advanced-layout.mdx#autocomplete-properties)
:::
...
:::tip
The default `layout-large.xml` in the Geocortex Mobile [quick start](sdk-quick-start.mdx) has examples of multiple panels that live in the taskbar.
:::
...
:::tip
The Geocortex Mobile SDK [quick start](sdk-quick-start.mdx) includes a large and small layout.
:::
...
:::important
The [quick start](Quick Start) does not ship with a medium layout configured. See [Add Medium Layout](usecases-add-medium-layout.mdx) for instructions on how to do this.
:::
...
Geocortex Mobile has a custom namespace `https://geocortex.com/layout/mobile/v1` in which components like `<gxm:taskbar >` live. You can add this namespace to your layout to use these components.
