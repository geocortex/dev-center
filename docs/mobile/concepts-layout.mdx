...
2. Tells the zoom widget to act upon [corresponding to the map it is placed in](concepts-advanced-layout.mdx#nested-model-imports)
...
:::tip
The [Quickstart](sdk-quick-start.mdx) comes with a schema for `layout.xml` which can [autocomplete slot values for a component](concepts-advanced-layout.mdx#autocomplete-properties)
:::
...
:::important
The [Quickstart](sdk-quick-start.mdx) does not ship with a medium layout configured. See [Add Medium Layout](usecases-add-medium-layout.mdx) for instructions on how to do this.
:::
