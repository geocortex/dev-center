...
:::important
It is vital that the event handles are cleaned up when the object is cleaned up, otherwise dangling references can occur. This can be done by implementing the `IDisposable` and `IDisposableTracker` interface, and then providing `this` as the second argument to a call to subscribe.
:::

```cs
public class CustomServiceOrComponent : ServiceBase, IDisposable, IDisposableTracker
{
    private bool disposed = false;

    public new IList<IDisposable> Disposables { get; } = new List<IDisposable>();
