...
In order to display in an Application, each component must display a `VisualElement`. This is usually accomplished through a [XAML View](TODO-link-xaml-view-component-reference). Each component can also optionally have an [associated configuration model](TODO-component-reference-configuration-model). This model represents settings from the component from the app config JSON, and the component is responsible for consuming this configuration model and passing the appropriate settings along to the view.
