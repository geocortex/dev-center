...
Check out the [Quick Start](quick-start) and [edit the default app config and layout](layout-config).
:::
...
## Linking Layout to App Configuration
...
:::note
If you do not provide a `config` attribute for a component, Mobile will attempt to infer a default value.
TODO - discuss this more in depth in advanced layout
:::
...
Every component can also have an `id` attribute. The `id` attribute uniquely identifies a component within a layout and allows app items to target the component. (TODO - @Jeff can you even target with a button? or only stuff like workflow)
