...
<!-- TODO - better link once SDK restructured into moving components into the SDK section -->

If you do not provide a `config` attribute for a component, Geocortex Web will use the [`createDefault` method](usecases-implement-component-participate-app-config.mdx#create-a-skeleton-app-config-definition) to generate the component.
:::
...
<layout xmlns="https://geocortex.com/layout/v1"
  ...
  <gxm:taskbar orientation="vertical">
    ...
    <map id="map-primary" config="myMapConfig" slot="main">
      // highlight-next-line
