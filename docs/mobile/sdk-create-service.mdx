...
Custom Services can register custom commands and operations. The following example shows how a custom service can register a command with the `Operations Registry`, and then how that command can be configured to run on a button press using the layout and app config.
...
namespace App1.Services
{
    ...
    {
        public CustomService(IOperationRegistry opReg)
            : base()
        {
            // highlight-start
            opReg.VoidOperation<string>("custom.alert").RegisterExecute((string customMessage) =>
            {
