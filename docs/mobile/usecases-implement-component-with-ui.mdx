...
In the file, add a new component class `CustomComponent` and register it with **Autofac**.
...
:::info
The two key components of this snippet are the `[assembly: Component(...)]` tag and the class extension `CustomComponent : ComponentBase`. These two pieces register the component with Geocortex Mobile and cause it to be initialized on startup.
:::

:::note
It's convention for the name of the component in the [`layout`](concepts-layout.mdx) to be lower case and kebab case, i.e. `custom-component`.
:::

## Add the Component to your Layout

First, the [layout](concepts-layout.mdx) needs to be told where to find your component. In order to find your component, you have to tell the layout what namespace it belongs too. In the assembly attribute of the `CustomComponent`, we declared it to be part of the `App1Namespace` which is defined in `XmlNamespaces.cs` We need to add this [xml namespace](concepts-layout.mdx#namespaces) as an attribute on the layout. In this example, it's added with the alias `custom`.
...
<layout ...>
  ...
  <gxm:taskbar>
    ...
    <panel>
        <custom:custom-component icon="warning"/>
    </panel>
  </gxm:taskbar>
  ...
</layout>
...
<Tabs defaultValue="component"
  values={[
    { label: 'CustomComponent.cs', value: 'component', },
    { label: 'CustomComponentView.xaml', value: 'view', },
    { label: 'CustomComponentView.xaml.cs', value: 'code-behind', }
  ]
}>
...
From here, you can implement your own business logic and interfaces, create custom [Commands and Operations](usecases-implement-command-operation.mdx) that are powered by and/or interact with your component, and much more. The [Xamarin Forms Documentation](https://docs.microsoft.com/en-us/xamarin/xamarin-forms/xaml/) has excellent guides to implementing user interfaces in Xaml.

:::info
Geocortex Mobile comes with built-in [Enhanced Components](https://github.com/geocortex/vertigis-mobile-samples/tree/master/Geocortex.Mobile.Samples/Geocortex.Mobile.Samples/Samples/Custom/GeocortexMobileElements) that can ease development and styling of custom UI.
:::
...
```csharp title="App1/App1/ProgressBarComponent.cs"
using App1;
using App1.Components;
using Geocortex.Mobile.Composition.Layout;
using System.Xml.Linq;
...
```xml title="App1/App1/ProgressBarComponentView.xaml
<?xml version="1.0" encoding="UTF-8"?>
<ContentView xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
             xmlns:d="http://xamarin.com/schemas/2014/forms/design"
...
```csharp title="App1/App1/ProgressBarComponentView.xaml.cs"
using App1.Components;
using Geocortex.Mobile.Composition;
...
```csharp title="App1/App1/ProgressBarComponentViewModel.cs"
using App1.Components;
using Geocortex.Mobile.Composition;
