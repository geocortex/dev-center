...
description: Geocortex Mobile - Learn how to configure mobile to change default application behavior
---
...
Many of the behaviors of Geocortex Mobile's built in [components](concepts-components-services.mdx) have configurable behaviors through the [app config](concepts-app-config.mdx). If you browse the `app.json` that is included with the [Quickstart](quick-start.mdx), you'll see properties like `action` or `onClick` in items that take an array of [commands and operations](concepts-commands-operations.mdx).
...
All behaviors defined in this manner are configurable. Commands or operations can be added or removed from the chain to modify its behavior. Further, it's possible to configure a `workflow.run` command that [executes custom behavior](change-default-search-behavior.mdx), or a [custom command and operation](implement-command-operation.mdx).
