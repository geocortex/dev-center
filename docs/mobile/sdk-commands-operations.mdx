...
When writing custom components you may want to take advantage of Geocortex Mobile's large [built-in suite of command and operations](api-commands-operations-events.mdx) or your own [custom commands and operations](tutorial-implement-command-operation.mdx).
...
```cs
class CustomService : ServiceBase, IDisposable, IDisposableTracker
{
...
```cs
class CustomComponent : ComponentBase, IDisposable, IDisposableTracker
{
    private bool disposed = false;
    ...
    {
        // ... implementation for run custom commanmd.
    }
