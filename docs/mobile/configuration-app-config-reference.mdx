...
App items in Geocortex Mobile correspond to a class that extends `VisualAppItem` and registers with **Autofac** as an `AppItem` type, like the `CustomComponentConfiguration` in the [app config use case](tutorial-implement-component-participate-app-config.mdx#create-a-skeleton-app-config-definition).
...
Registering a class in this manner makes mobile aware of a new **app item type**, which can be added to the items list in the `app.json` with the appropriate `$type` specified. Components can consume these app items, and by extension, the configuration. Check out [components with app config](tutorial-implement-component-participate-app-config.mdx#create-a-skeleton-app-config-definition) for more details.
