...
App items in Geocortex Mobile correspond to a class that extends `VisualAppItem` and registers with **Autofac** as an [`AppItem` type](sdk-dependency-injection.mdx#appitem), like the `CustomComponentConfiguration` in the [app config use case](tutorial-implement-component-participate-app-config.mdx#create-a-skeleton-app-config-definition).
