...
Registering a class in this manner makes mobile aware of a new **app item type**, which can be added to the items list in the `app.json` with the appropriate `$type` specified. Components can consume these app items, and by extension, the configuration. Check out [the component reference](sdk-component-reference.mdx#configuration) for more details.
