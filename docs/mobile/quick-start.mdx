...
-   Visual Studio 2019 or higher on Windows is the only officially supported development enviroment for the Geocortex Mobile SDK. Other development environments may be possible, but have not been verified.
...
Each platform has platform specific setup and an entry point, after which they all boostrap the application through the platform agnostic project. In the shared App1 project, the `OnStart` method of `App.cs` calls the Geocortex Mobile SDK Bootstrapper, and loads and displays the application with the given `app.json` configuration.
