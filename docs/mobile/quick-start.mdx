...
If the built in configurability of Geocortex Mobile does not serve your purposes, then it might be time to dive into the Geocortex Mobile SDK. Building applications on top of the Geocortex Mobile SDK requires enaging in the development, testing, and deployment of a custom application, so it's worthwhile to make sure your problem cannot be solved [with workflow or other advanced configuration](overview.mdx#start-with-your-use-case).
...
-   Set **Geocortex.Mobile.Viewer.Windows** as your startup project.
-   Change the architecture from _Any CPU_ to _x86_ or _x64_.
-   Build and run.

### Build and run for Android

-   Set **Geocortex.Mobile.Viewer.Droid** as your startup project.
...
-   Set **Geocortex.Mobile.Viewer.iOS** as your startup project.
...
-   `Geocortex Viewer Framework`
    -   This package provides the infrastructure that powers the [layout](concepts-layout.mdx) and [app config](concepts-app-config.mdx) that is central to every Mobile App.

TODO: Packages - wait on Kenny package publishing -->
...
The quick start is a useful starting point for your custom Geocortex Mobile application. Build on top of it by following along with a [use case](implement-command-operation.mdx), or check out the [SDK Samples](https://github.com/geocortex/mobile-sdk-samples) for inspiration and examples of what you can accomplish with the Geocortex Mobile SDK.
