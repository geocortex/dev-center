...
:::note
Classes instantiated in Mobile via [dependency injection](sdk-dependency-injection.mdx) will automatically be disposed by Autofac, and do not need to be disposed of manually by the developer.
:::
...
`ComponentBase` and `ServiceBase` already implement `IDisposableTracker`, so any custom components or services extending these base classes can take advantage of this functionality right away. Here is a brief example of how to use it in a new class:
