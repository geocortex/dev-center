...
This will override the default arugment passing behaviour for that command or operation. Each command has typed arguments that can will have [autocomplete linting in the app.json](concepts-advanced-config#autocomplete-properties). For example, `map.rotate-by` takes a object of type `MapRotateArgs`, which has the property `Rotation` of type `double`. In the app config, the `Rotation` property can be camelCase'd instead of PascalCased.
...
`tasks.identify` receives this argument, and since it is an operation, produces an output. Looking at the [Commands and Operations Documentation](api-commands-operations), `tasks.identify` has output with the shape
