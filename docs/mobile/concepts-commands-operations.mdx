...
This will override the default argument passing behavior for that command or operation. Each command has typed arguments that can will have [autocomplete linting in the app.json](concepts-advanced-config.mdx#schema-properties). For example, `map.rotate-by` takes a object of type `MapRotateArgs`, which has the property `Rotation` of type `double`. In the app config, the `Rotation` property can be camelCase'd instead of PascalCased.
