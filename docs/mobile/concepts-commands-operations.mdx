...
[Components or Services](concepts-components-services) are usually responsible for registering a command or operation with Geocortex Mobile. Once a command or operation has been registered, it can be used from anywhere in the application. [Custom Commands and Operations](implement-command-operation) are registered with [Custom Services](implement-background-service) or [Custom Components](implement-component-with-ui) in this manner.
...
Since these commands and operations are running from the [context](TODO - app context link) of a Feature Details component, the first command or operation will receive the relevant feature as it's input. If the property is a command chain, the next behaviour in line will receieve one of two possible inputs.
...
This will override the default arugment passing behaviour for that command or operation. Each command has typed arguments that can be browsed in the [API](TODO - api link here). For example, `map.rotate-by` takes a object of type [`MapRotateArgs`](TODO link to api), which has the property `Rotation` of type `double`. In the app config, the `Rotation` property can be camelCase'd instead of PascalCased.
...
`tasks.identify` does not have any named arguments defined, so it will take arguments passed into it from it's current [context](TODO - link to context). Since this chain is run on a map click, the context argument passed in has the shape
...
:::tip
Check out [Change Default Behaviour](change-default-behaviour) for more ways to override default behaviour like a Map's on click event.  
:::
