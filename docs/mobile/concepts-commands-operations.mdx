...
In Mobile, [Components or Services](concepts-components-services) are usually responsible for registering a Command/Operation with Mobile. Once a Command/Operation is registered, it can be used from anywhere in the application. [Custom Commands and Operations](implement-command-operation) can be registered with [Custom Services](implement-background-service) or [Custom Components](implement-component-with-ui) in the same way.
