...
**Resource URI's** are used to load layouts in Geocortex Mobile. Layout files are [added to Geocortex Mobile](add-medium-layout) as an asset which is copied into the end users local data folder. These copied assets are referenced with a URI of the format
...
App items in Geocortex Mobile correspond to a class that extends `VisualAppItem` and registers with **autofac** as an `AppItem` type, like the `HarmlessComponentConfiguration` in the [app config use case](implement-component-participate-app-config#create-a-skeleton-app-config-definition).
