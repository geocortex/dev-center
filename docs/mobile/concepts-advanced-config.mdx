...
The two properties `$schema` and `schemaVersion` are almost always present in an `app.json` file. They power the Visual Studio intellisense in the `app.json` file for the various properties that each **app item** has. The `app.json` included with the [Quickstart](sdk-getting-started.mdx) references a schema at the root of the project. If you move an `app.json` file, you will need to fix the relative path to the schema file. For more information on json schemas, see [json-schema.org](https://json-schema.org/).
