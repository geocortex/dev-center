...
Many of the behaviors of Geocortex Mobile's built in [components](concepts-components-services.mdx) have behaviours which can be configured through [app config](concepts-app-config.mdx). If you browse the `app.json` that is included with the [Quick Start](sdk-quick-start.mdx), you'll see properties like `action` or `onClick` in items that take an array of [commands and operations](concepts-commands-operations.mdx).
...
All behaviors defined in this manner are configurable. Commands or operations can be removed from the chain, and any behavior can be replaced with a `workflow.run` command that [executes custom behavior](usecases-change-default-search-behavior.mdx), or even with [custom commands and operations](usecases-implement-command-operation.mdx).
