...
Many of the behaviors of Geocortex Mobile's built in [components](concepts-components-services.mdx) have configurable behaviors through the [app config](concepts-app-config.mdx). If you browse the `app.json` that is included with the [Quickstart](sdk-quick-start.mdx), you'll see properties like `action` or `onClick` in items that take an array of [commands and operations](concepts-commands-operations.mdx).
...
All behaviors defined in this manner are configurable. Commands or operations can be added or removed from the chain to modify its behavior. Further, it's possible to configure a `workflow.run` command that [executes custom behavior](usecases-change-default-search-behavior.mdx), or a [custom command and operation](usecases-implement-command-operation.mdx).
...
<!-- <UseCaseContainer>
    <UseCaseCard
        title="Configuring Basic Commands"
        link="https://github.com/geocortex/vertigis-mobile-samples/tree/master/Geocortex.Mobile.Samples/Geocortex.Mobile.Samples/Samples/AppConfiguration/Commands"
        linkText="Check it out"
    />
    <UseCaseCard
        title="Configuring Map and Feature Commands"
