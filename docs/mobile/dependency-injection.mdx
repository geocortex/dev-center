...
Geocortex Mobile relies heavily on inversion of control patterns to inject dependencies into it's [components and services](concepts-components-services). Under the hood, Geocortex Mobile uses [autofac](https://autofaccn.readthedocs.io/en/latest/index.html) to register and inject components, services and other classes. To inject a dependency into a class in Geocortex Mobile, two things must happen.
...
A good example of this pattern is the `AlertOperations` class in the [commands and operation](implement-command-operation) use case.
...
Used for registering a component that **does not** consume configuration. For a more detailed example, see the use case on [implementing a custom component](implement-component-with-ui#create-a-component-skeleton).
...
Used for registering a component that **does** consume configuration. For a more detailed example, see the use case on [implementing a custom component with configuration](implement-component-participate-app-config#consume-the-configuration-in-the-component).
...
Used for registering a service. For a more detailed example, see the use case on [implementing a custom service](implement-service#create-a-new-service).
...
Used for registering the definition for an [app item](concepts-app-config#anatomy-of-an-app-config) with Geocortex Mobile. For a more detailed example, see the use case on [implementing a custom component with app configuration](implement-component-participate-app-config#create-a-skeleton-app-config-definition).
...
Used for registering a [view](https://docs.microsoft.com/en-us/xamarin/xamarin-forms/enterprise-application-patterns/mvvm) with Geocortex Mobile. For a more detailed example see the use case on [implementing a custom component with UI](implement-component-with-ui#use-xaml-to-define-your-ui).
...
Used for registering a [view model](https://docs.microsoft.com/en-us/xamarin/xamarin-forms/enterprise-application-patterns/mvvm) with Geocortex Mobile. For a more detailed example see the use case on [implementing a custom component with UI](implement-component-with-ui#example-doomsday-cookie-clicker).
