...
[Components or Services](configuration-components-services.mdx) are usually responsible for registering a command or operation implementation with Geocortex Mobile. Once a command or operation has been registered, it can be used from anywhere in the application. [Custom Commands and Operations](tutorial-implement-command-operation.mdx) can be registered with [Custom Services](tutorial-implement-service.mdx) or [Custom Components](tutorial-implement-component-with-ui.mdx) in this manner.
...
Each command has typed arguments that will [autocomplete in the app.json](configuration-advanced-app-config.mdx#schema-properties). For example, `map.rotate-by` takes a object of type `MapRotateArgs`, which has the property `Rotation` of type `double`. In the app config, the `Rotation` property can be camelCased instead of PascalCased.
