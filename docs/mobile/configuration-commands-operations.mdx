...
Commands and operations are used to power much of Geocortex Web's built in behavior and interactions. They can be run through configuration, or through [custom services](sdk-services-commands-operations.mdx) or [components](sdk-components-commands-operations.mdx).
...
Checkout this example of [running a workflow from app config](tutorial-run-workflow-app-config.mdx).
